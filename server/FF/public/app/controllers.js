 ...
 */
/**
 * Represents angular controllers
 *
 * @version 1.0
 * @author TCSASSEMLBER
 */
...
var appControllers = angular.module('MomAndPop.controllers', ['ngCookies']);
...
appControllers.controller('ChampionHomeCtrl', ['$scope', '$http', function ($scope, $http) {
    $scope.resetGlobal({
    ...
    function searchGiftCardOffers(params){
        $http({
            method: 'GET',
        ...
        .success(function(offerData) {
            $scope.giftCardOffers = offerData.items;

            $http({
                method: 'GET',
                url: config.REST_SERVICE_BASE_URL + '/users/me/giftCards',
                headers: {
                    Authorization: 'Bearer ' + $scope.getSessionToken()
                },
                params: params
            })
            .success(function(giftCardData) {
                angular.forEach(giftCardData.items, function(giftCard){
                    ...
                    for (var i = $scope.giftCardOffers.length - 1; i >= 0; i--) {
                        ...
                        if(giftCarOffer.id === giftCard.giftCardOfferId){
    ...
    searchGiftCardOffers(queryParams);
    ...
    $scope.setOrder = function (order) {
        ...
        searchGiftCardOffers(params);
    };

    $scope.doGiftCardSearch = function (name) {
        var params = {
            sortOrder: 'Ascending',
            sortBy: 'id',
            businessName: name
...
appControllers.controller('detailsPage', ['$scope', '$http', '$routeParams', function ($scope, $http, $routeParams) {
    ...
    function getGiftCardDetail(giftCardId){
        $http({
            method: 'GET',
            url: config.REST_SERVICE_BASE_URL + '/users/me/giftCards/' + giftCardId,
        })
    ...
    getGiftCardDetail($routeParams.id);
}]);
...
//Login Controller
appControllers.controller("loginCtrl", ['$scope', '$http', '$location', '$rootScope', '$cookies', function ($scope, $http, $location, $rootScope, $cookies) {
    //Validate
    $scope.resetGlobal({
        headless: true,
    ...
    $scope.validate = function() {
        ...
        var plainAuth = btoa($scope.username + ':' + $scope.password);

        $http({
            method: 'POST',
            url: config.REST_SERVICE_BASE_URL + '/login?type=password',
            headers: {
                Authorization: 'Basic ' + plainAuth
            }
        })
        .success(function(data, status) {
            $scope.setSessionToken(data.sessionToken);

            if ($scope.isRememberMe) {
                $cookies.token = data.sessionToken;
            } else {
                delete $cookies.token;
            }
            $rootScope.login = 'user.json';
